...
struct PausePunctuationProcessorTests {
    ...
    @Test func longPauseInsertsPeriod() {
        let timings = [
            ...
            WordTimingInfo(word: "world", start: 2.5, end: 3.0),
        ]
    ...
    @Test func questionWordAfterPause() {
        let timings = [
            ...
            WordTimingInfo(word: "what", start: 2.0, end: 2.3),
            WordTimingInfo(word: "happened", start: 2.4, end: 2.8),
        ]
    ...
    @Test func continuationWordDowngradesComma() {
        ...
        // Using only 3 timings → fewer than 5 so default thresholds apply:
        // sentenceBreak=1.5, clauseBreak=0.4. Gap of 1.6 >= 1.5 → sentence break.
        ...
        let timings = [
            ...
            WordTimingInfo(word: "and", start: 2.1, end: 2.4),
            WordTimingInfo(word: "finished", start: 2.5, end: 2.9),
        ]
    ...
    @Test func adaptiveThresholdsWithManyWords() {
        ...
        // median = 0.1, sentenceBreak = max(0.3, 1.0) = 1.0, clauseBreak = max(0.15, 0.2) = 0.2
        // Gap of 2.0 between word5 and word6 >= 1.0 → period inserted
        let timings = [
    ...
    @Test func gerundMidSentenceNoPeriod() {
        // "using" ends in "-ing" → period suppressed even with a long pause
        let timings = [
            WordTimingInfo(word: "using", start: 0.0, end: 0.4),
            WordTimingInfo(word: "a",     start: 2.0, end: 2.1),
            WordTimingInfo(word: "thing", start: 2.2, end: 2.6),
        ]
        let result = processor.process("using a thing", context: makeContext(timings: timings)).text
        #expect(!result.contains("."))   // no sentence break after gerund
    }
}
